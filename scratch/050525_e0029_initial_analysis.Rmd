...
### Plots!!! (FOR NOT ONLY ON REPLICATE 1)
...
mixture_ASVs_XBBXBA_colonized <- colonization_results %>% 
  filter(mixture == "XBB-029+XBA-029", replicate == 1) %>% 
  ggplot(aes(x = mixture, y = relAbundance, fill = Family, alpha = factor(colonizer))) +
  geom_bar(stat = "identity", color = "black") +
  scale_fill_manual(values = my_colors) +
...
subject_composition_plot_day29 <- subject_composition("XCB-029+XBA-029", "XCB-029", "XBA-029", 1) + theme_cowplot()
subject_composition_plot_day36 <- subject_composition("XCB-029+XBA-036", "XCB-029", "XBA-036", 1) + theme_cowplot()
subject_composition_plot_day64 <- subject_composition("XCB-029+XBA-064", "XCB-029", "XBA-064", 1) + theme_cowplot()
...
ggsave(
  filename = file.path(OUTPUTDIR, "compositions_XBAXCB.png"),
  plot = combined_plot,
  width = 10,
  height = 10,
  dpi = 600,
...
total_colonization <- colonization_results %>% 
  group_by(mixture) %>% 
  summarize(total_colonizers = sum(colonizer)) %>% 
  mutate(day = str_sub(mixture, -2))


# Plot
colonization_day_plot <- 
