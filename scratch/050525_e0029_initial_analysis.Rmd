---
title: "e0029_initial_analysis"
output: html_document
date: "2025-05-06"
---


# Load in packages

```{r, warning=FALSE}
library(tidyverse)
library(dplyr)
library(ggplot2)
library(readr)
library(ggrepel)
library(purrr)
```




# Load in the Household Project dataset

```{r}

household_data <- read.table("C:/Users/anaes/OneDrive/UCI_Spring25/rotation/e0026-e0029-e0030.txt", header = TRUE)

# Divide dataset into separate tables by experiment
e0029 <- filter(household_data, str_detect(household_data$sample, "e0029"))

e0026 <- filter(household_data, str_detect(household_data$sample, "e0026"))

# Extract subject, day, household information
e0029_clean <- e0029 %>%
  mutate(
    subject = str_sub(biosample1, 1, -5),
    day = str_sub(biosample1, -3),
    household = str_sub(biosample1, 1, -6)
  )

# Specify output directory

OUTPUTDIR <- "./outputs"

# Define color palette

my_colors <- readRDS("C:/Spring-Rotation-2025/scratch/familyColorPalette.rds") 

```


### First plot composition across mixtures by biosample2 (donor community)


```{r}

# First get only the biosamples corresponding to actual mixtures
composition_across_mixes <- e0029 %>%
  # filter(!is.na(relAbundance)) %>%
  group_by(biosample2, replicate) %>%
  mutate(
    biosample1 = factor(biosample1, levels = unique(biosample1))  # sets the order of levels to match the order they appear within the current group
  ) %>%
  ungroup() # Remove the group structure to avoid it affecting during plotting



composition_across_mixes_plot <- ggplot(composition_across_mixes, aes(x = biosample1, y = relAbundance, fill = Family)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = my_colors) +
  labs(
    title = "Community Composition Across Mixtures",
    x = "Biosample1",
    y = "Total Relative Abundance"
  ) +
  theme_gray() + 
  theme(
    legend.position = "none",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 10),
    strip.text.x = element_text(angle = 90, hjust = 0, size = 15)
  ) +
  facet_grid(replicate ~ biosample2, scales = "free_x", space = "free_x")



ggsave(
  filename = file.path(OUTPUTDIR, "composition_across_mixes.png"),
  plot = composition_across_mixes_plot,
  width = 25,
  height = 10,
  dpi = 300
)

```



### Try looking at the opposite: composition across mixtures by biosample1 (donor community)


```{r}

# First get only the biosamples corresponding to actual mixtures. For blanks, only get the first well of the duplicates
composition_across_mixes <- e0029 %>%
  filter(
    !(biosample2 == "blank") |
    (biosample2 == "blank" & !well %in% c("G11", "H11"))) %>%
  filter(
    biosample1 != "blank" | 
    (plate == "e0029.B.5" & well %in% c("G12", "H12"))
  ) %>% 
  group_by(biosample1, replicate) %>%
  mutate(
    biosample2 = factor(biosample2, levels = unique(biosample2))
  ) %>%
  ungroup()



# Why do the blank mixtures have an so many more observations?
composition_across_mixes %>%
  distinct(biosample1, biosample2, replicate) %>%  # ensure unique pairs
  group_by(biosample1) %>%
  summarise(n_biosample2 = n_distinct(biosample2)) %>%
  arrange(desc(n_biosample2))


library(forcats)

ggplot(
  composition_across_mixes,
  aes(x = forcats::fct_relevel(factor(biosample2), "blank"), y = relAbundance, fill = Family)
) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = my_colors) +
  labs(
    title = "Community Composition Across Mixtures",
    x = "Biosample2",
    y = "Total Relative Abundance"
  ) +
  theme_gray() + 
  theme(
    legend.position = "none",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 10),
    strip.text.x = element_text(angle = 90, hjust = 0, size = 15)
  ) +
  facet_grid(replicate ~ biosample1, scales = "free_x", space = "free_x")



ggsave(
  filename = file.path(OUTPUTDIR, "composition_across_mixes_flipped.png"),
  plot = composition_across_mixes_plot,
  width = 25,
  height = 10,
  dpi = 300
)

```




### Question #1: Was colonization resistance affected by antibiotic treatment and did it recover by day 64?

Pseudocode:

Group by mixture

- For each of these unique communities, extract all OTU identities

- For the recipient communities we can do the same by looking at biosample1+blank mixtures?

- For the donor communities we have to go back to the e0026 dataset and extract the OTUs from there

- Recalculate relative abundances for datasets (since there are replicates) and b-mixes and filter for those asvs >0.0001

**Only use ASVs with relAbundance > 0.001**

```{r}

# Have filter for relative abundance in actual calculations to avoid losing information
# Only extract OTU

limit_of_detection <- 0.001

```


**Create a mixture ASVs, recipient ASVs, and donor ASVs dataset**

```{r}

# Get mixture communities
mixture_ASVs1 <- e0029 %>% 
  filter(relAbundance > limit_of_detection) %>% 
  filter(replicate == 1)


mixture_ASVs2 <- e0029 %>% 
  filter(relAbundance > limit_of_detection) %>% 
  filter(replicate == 2)

# Get recipient communities
recipient_ASVs1 <- e0029 %>% 
  filter(relAbundance > limit_of_detection) %>%  
  filter(biosample2 == "blank", replicate == 1)

recipient_ASVs2 <- e0029 %>% 
  filter(relAbundance > limit_of_detection) %>%  
  filter(biosample2 == "blank", replicate == 2)


# Extract donor community IDs from e0029
donor_communities <- e0029 %>%
  pull(biosample2) %>%
  unique()

# Extract single donor OTUs from e0026
single_donor_ASVs <- e0026 %>%
  filter(biosample1 %in% donor_communities, passage == "8") %>% 
  
# We'll deal with the b-mix and two-donor communities later
donor_combinations <- list(
  c("XBB-029", "XKB-029"),
  c("XCB-029", "XBB-029"),
  c("XDB-029", "XCB-029"),
  c("XEB-029", "XDB-029"),
  c("XFB-029", "XEB-029"),
  c("XGB-029", "XFB-029"),
  c("XHB-029", "XGB-029"),
  c("XIB-029", "XHB-029"),
  c("XJB-029", "XIB-029")
)

# donor_combinations[[1]] 
#   
# two_donor_ASVs <- single_donor_ASVs %>% 
#   filter(biosample1 %in% donor_combinations[[1]])
# 
# combined_otus <- two_donor_ASVs %>% 
#   group_by(OTU) %>% 
#   summarize(total_abundance = sum(relAbundance))
# 
# combined_otus <- combined_otus %>% 
#   mutate(avg_abundance = total_abundance/2)
# 
# 
# combined_otus <- combined_otus %>% 
#   mutate(relAbundance_final = avg_abundance/sum(avg_abundance)) %>% 
#   select(OTU, relAbundance_final) %>% 
#   filter(relAbundance_final > limit_of_detection)




```




```{r}
# Loop and collect into one dataframe. map_dfr binds all the donor pair dataframes together at the end. Iterates over each element in the list donor_combinations (i.e., each donor pair). Applies the anonymous function(donor_pair) { ... } to each element. Combines all returned dataframes by row (dfr = data frame row-binding)

two_donor_ASVs <- map_dfr(donor_combinations, function(donor_pair) {
  pair_name <- paste(donor_pair, collapse = "+")
  
  two_donor_ASVs <- single_donor_ASVs %>%
    filter(biosample1 %in% donor_pair)
  
  combined_otus <- two_donor_ASVs %>%
    group_by(OTU) %>%
    summarize(total_abundance = sum(relAbundance), .groups = "drop") %>%
    mutate(
      avg_abundance = total_abundance / 2,
      relAbundance_final = avg_abundance / sum(avg_abundance)
    ) %>%
    select(OTU, relAbundance_final) %>%
    filter(relAbundance_final > limit_of_detection) %>%
    mutate(donors = pair_name)  # Add mixture ID
  
  return(combined_otus) # donor pair data frame
})



```





```{r}
# Calculate colonization
get_colonization <- function(mix_asvs, donor_asvs, recipient_asvs) {
  # Find OTUs shared by donor and mixture
  shared_asvs <- intersect(single_donor_ASVs, mixture_ASVs1)
  
  # print(paste("donor asvs", donor_asvs))
  
  # Colonized if at least one shared ASV is not in recipient
  colonized <- any(!(shared_asvs %in% recipient_asvs))
  
  return(as.integer(colonized))  # 1 if colonized, 0 if not
}


# Recalculate relative abundance

```































**Get mixture ASVs**

```{r}
  
# Replicate 1
mixture_ASVs_rep1 <- e0029_colonization %>%
  filter(replicate == 1) %>% 
  select(mixture, OTU) %>%
  distinct() %>%                        
  group_by(mixture) %>%
  summarise(asvs = list(unique(OTU)), .groups = "drop") %>%
  deframe()

mixture_ASVs_rep1[["XBB-029+XBA-029"]]

#Replicate 2
mixture_ASVs_rep2 <- e0029_colonization %>%
  filter(replicate == 2) %>% 
  select(mixture, OTU) %>%
  distinct() %>%                        
  group_by(mixture) %>%
  summarise(asvs = list(unique(OTU)), .groups = "drop") %>%
  deframe()

```


**Get recipient ASVs**

```{r}



# Replicate 1
recipient_ASVs_rep1 <- e0029_colonization %>% 
  filter(replicate == 1, biosample2 == "blank", biosample1 != "blank") %>% 
  select(biosample1, OTU) %>%
  distinct() %>%                        
  group_by(biosample1) %>%
  summarise(asvs = list(unique(OTU)), .groups = "drop") %>%
  deframe()

# Replicate 2
recipient_ASVs_rep2 <- e0029_colonization %>% 
  filter(replicate == 2, biosample2 == "blank", biosample1 != "blank") %>% 
  select(biosample1, OTU) %>%
  distinct() %>%                        
  group_by(biosample1) %>%
  summarise(asvs = list(unique(OTU)), .groups = "drop") %>%
  deframe()

# recipient_ASVs_rep1[["XBA-029"]]

```


**Get donor ASVs**

```{r}

# Extract only the donor communities
donor_communities <- e0029_colonization %>%
  # filter(!is.na(biosample2), biosample2 != "blank") %>%
  pull(biosample2) %>%
  unique()


# Extract single donor OTUs from e0026
donor_ASVs <- e0026_colonization %>% 
  filter(biosample1 %in% donor_communities, passage == "8") %>% 
  select(biosample1, OTU) %>%
  distinct() %>%
  group_by(biosample1) %>%
  summarise(asvs = list(unique(OTU)), .groups = "drop") %>%
  deframe()


# Add b-mix info by computing the total asvs in the donor samples
b_mix_asvs <- Reduce(union, donor_ASVs)
donor_ASVs[["b-mix"]] <- b_mix_asvs


# Infer mixture donor OTUs by adding together each pair:
donor_combinations <- list(
  "XBB-029+XKB-029" = c("XBB-029", "XKB-029"),
  "XCB-029+XBB-029" = c("XCB-029", "XBB-029"),
  "XDB-029+XCB-029" = c("XDB-029", "XCB-029"),
  "XEB-029+XDB-029" = c("XEB-029", "XDB-029"),
  "XFB-029+XEB-029" = c("XFB-029", "XEB-029"),
  "XGB-029+XFB-029" = c("XGB-029", "XFB-029"),
  "XHB-029+XGB-029" = c("XHB-029", "XGB-029"),
  "XIB-029+XHB-029" = c("XIB-029", "XHB-029"),
  "XJB-029+XIB-029" = c("XJB-029", "XIB-029")
)



extended_donor_ASVs <- donor_ASVs

for (combo_name in names(donor_combinations)) {
  component_donors <- donor_combinations[[combo_name]]
  
  # Combine OTUs using union() across selected donors
  combined_otus <- Reduce(union, donor_ASVs[component_donors])
  
  # Add to the extended list
  extended_donor_ASVs[[combo_name]] <- combined_otus

}

# Add "blank" as a donor
extended_donor_ASVs[["blank"]] <- mixture_ASVs_rep1[["blank+blank"]]


all_donor_asvs <- unique(unlist(donor_ASVs))
length(all_donor_asvs)

```



### Define a function to categorize colonization (one-by-one test)


```{r}

get_colonization <- function(mix_asvs, donor_asvs, recipient_asvs) {
  # Find OTUs shared by donor and mixture
  shared_asvs <- intersect(donor_asvs, mix_asvs)
  
  # print(paste("donor asvs", donor_asvs))
  
  # Colonized if at least one shared ASV is not in recipient
  colonized <- any(!(shared_asvs %in% recipient_asvs))
  
  return(as.integer(colonized))  # 1 if colonized, 0 if not
}


get_colonization(
  mix_asvs = mixture_ASVs_rep1[["b-mix+XEA-036"]],
  donor_asvs = extended_donor_ASVs[["b-mix"]],
  recipient_asvs = recipient_ASVs_rep1[["XEA-036"]]
)
# returns 1 or 0

```












### Full function looping through mixture ids

```{r}
# Initialize results list
colonization_results <- list()

# Loop through each mixture
for (mix_id in names(mixture_ASVs_rep1)) {
  # Split into donor and recipient
  ids <- unlist(strsplit(mix_id, "\\+"))
  
  donor_id <- ids[1]
  recipient_id <- ids[2]
  
  print(paste("mix id", mix_id))
  print(paste("donor id", donor_id))
  print(paste("recipient", recipient_id))
  
  # Extract ASV lists
  mix_asvs <- mixture_ASVs_rep1[[mix_id]]
  donor_asvs <- extended_donor_ASVs[[donor_id]]
  recipient_asvs <- recipient_ASVs_rep1[[recipient_id]]
  
  # print(paste("mix asvs", mix_asvs))
  # print(paste("donor asvs", donor_asvs))
  # print(paste("recipient asvs", recipient_asvs))
  
  # Apply the function
  colonized_flag <- get_colonization(
    mix_asvs = mix_asvs,
    donor_asvs = donor_asvs,
    recipient_asvs = recipient_asvs
  )

  # Store result
  colonization_results[[mix_id]] <- data.frame(
    mixture = mix_id,
    donor = donor_id,
    recipient = recipient_id,
    colonized = colonized_flag,
    stringsAsFactors = FALSE
  )
}

# Combine into final data frame
colonization_df <- do.call(rbind, colonization_results)



```


