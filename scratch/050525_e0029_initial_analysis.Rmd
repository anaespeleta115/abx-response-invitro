...
colonization_results <- foreach(mix_id = mixture_ids, .combine = bind_rows) %do% {
  ...
  # if (length(ids) != 2) {
  #   warning(paste("Invalid mixture ID:", mix_id))
  #   return(NULL)
  # }
...
preantibiotic_plot <- plot_grid(
  ggdraw() + draw_label("Pre-abx (Rep 1)", fontface = "bold", size = 12, hjust = 0.5),
  row_of_plots,
  ncol = 1,
...
postantibiotic_plot <- plot_grid(
  ggdraw() + draw_label("Post-abx (Rep 1)", fontface = "bold", size = 12, hjust = 0.5),
  row_of_post_plots,
  ncol = 1,
  rel_heights = c(0.1, 1)
)
...
postantibiotic64_plot <- plot_grid(
  ggdraw() + draw_label("Post-abx day 64 (Rep 1)", fontface = "bold", size = 12, hjust = 0.5),
  row_of_post_plots,
  ncol = 1,
  rel_heights = c(0.1, 1)
)
...
**NOTE! CHECK OUT PATCHWORK PACKAGE**
