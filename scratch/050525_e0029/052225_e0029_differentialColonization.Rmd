...
subject_composition <- function(mix_id, donor_id, recipient_id, replicate) {
  ...
  mixture_ASVs_colonized <- actual_colonizers_results %>% 
    filter(mixture == mix_id, day == "036", replicate == 1) %>% 
...
subject_composition_plot_day29 <- subject_composition("XJB-029+XBA-029", "XJB-029", "XBA-029", 1) + theme_cowplot()
subject_composition_plot_day36 <- subject_composition("XJB-029+XBA-036", "XJB-029", "XBA-036", 1) + theme_cowplot()
...
ggsave(
  filename = file.path(OUTPUTDIR, "compositionsXJBXBA_lost.png"),
  plot = combined_plot,
  width = 20,
  height = 10,
  dpi = 600,
...
recipient_lost <- recipient_lost %>% 
  filter(biosample1 == "XBA-029")

single_donor_ASVs <- single_donor_ASVs %>% 
  filter(biosample1 == "XBB-029")


single_sample_actual_colonizers <- actual_colonizers_results %>% 
  filter(mixture_pair == "XBB+XBA", actual_colonizer == 1, day != "064") %>% 
  distinct(Family, OTU)
...
bootstrap_results <- map_dfr(1:n_trials, function(trial) {
  ...
  otu_shared <- length(intersect(boot_sample$OTU, recipient_lost$OTU)) # get shared taxa between the bootstrap population and the recipient population
...
single_sample_diff_colonizers <- actual_colonizers_results %>% 
  filter(mixture_pair == "XBB+XBA", diff_colonizer == 1, day != "064") %>% 
  distinct(Family, OTU)


observed_otus <- length(intersect(single_sample_diff_colonizers$OTU, recipient_lost$OTU))
...
intersect(single_sample_diff_colonizers$Family, recipient_lost$Family)
```
