...
```{r}
otu_level_data <- bind_rows(colonization_list, .id = "mixture")

```



**Loop through all mixtures to get colonization results**
...
colonization_day_plot2 <- 
  ggplot(total_colonization, aes(x = factor(day), y = prop_colonizer, fill = factor(day))) +
  geom_boxplot() +
  geom_jitter(position = position_jitterdodge(jitter.width = 0.2), size = 1, alpha = 0.6) +
  labs(
