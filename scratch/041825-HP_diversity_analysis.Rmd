...
# Significance of antibiotic on the entire data

wilcox.test(Shannon ~ antibiotic, data = e0026_alpha)


# Significant of antibiotic on diversity by day

wilcox_results_by_day <- e0026_alpha %>%
  group_split(day) %>%
  map_df(~{
    data <- .
    result <- wilcox.test(Shannon ~ antibiotic, data = data)
    tibble(
      day = unique(data$day),
...
ggsave("beta_time.png", width = 14, height = 12, dpi = 300, bg = "white")
...
ggsave("jsd_p0p8_plot.png", width = 14, height = 12, dpi = 300, bg = "white")
