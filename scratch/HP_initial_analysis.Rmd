...
ggplot(XSA036_household_data, aes(x = reorder(Family, -total_reads), y = log(total_reads), fill=Phylum))+
  geom_histogram(stat="identity")+
  labs(title = "Family read distribution in Sample XSA-036", x= "Family Identity", y = "Read Count")+
  theme_light()+
...
ggplot(XTA029_household_data, aes(x = reorder(Family, -total_reads), y = log(total_reads), fill=Phylum))+
  geom_histogram(stat="identity")+
  labs(title = "Family read distribution in Sample XTA-029", x= "Family Identity", y = "Read Count")+
  theme_light()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
# group by antibiotic group
# from there, 
e0026 %>% 
  group_by(biosample1) %>% 
  summarise(richness = sum(e0026$relAbundance > 0.1))
