...
for (mix in mixture_ids) {
  ...
  donor <- ids[1]
  donor_id <- str_sub(donor, 1, -5)
  recipient <- ids[2] 
  recipient_id <- str_sub(recipient, 1, -5)
  mix_pair <- paste(donor_id, recipient_id, sep = "+")
  
  
  # Get all potential colonizers for that mixture
  single_sample_potential_colonizers <- get_potential_colonizers(mix, donor, recipient, replicate)
  ...
  if (nrow(single_sample_potential_colonizers) == 0 || nrow(single_recipient_lost) == 0) {
    next
  }
  
  # Bootstrap setup
  n_trials <- 1000
  n_sample <- nrow(single_sample_diff_colonizers) # CHANGE THIS!!!
  
  bootstrap_results <- map_dfr(1:n_trials, function(trial) {
    boot_sample <- single_sample_potential_colonizers %>%
...
enrichment_summary_36 <- left_join(observed_rows, enrichment_pvals, by = "mixture") %>%
  mutate(
