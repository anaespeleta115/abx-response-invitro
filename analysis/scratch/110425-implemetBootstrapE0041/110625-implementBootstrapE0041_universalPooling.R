...
# Script that implements Bootstrap test with modification to pool from universal colonizers.
...
for (mix in mixture_ids) {
  ...
  # Get universal colonizers between pre-abx and post-abx-V1
  sample_universal_colonizers <- mixture_colonization_full %>% 
    filter(mixture == mix, recipient == "post-abx-V1", colonized_pre_abx == 1, colonized_post_abx_v1 == 1) %>% 
    distinct(OTU, relAbundance, Family)
  ...
  bootstrap_results <- map_dfr(1:n_trials, function(trial) {
    boot_sample <- sample_universal_colonizers %>%  # here is where I changed the pooling
      slice_sample(n = n_sample, weight_by = relAbundance, replace = TRUE) # take a slice out of the universal colonizers and see if they
...
savePNGPDF(paste0(OUTDIR, "manhattan_plot_V1_universal_pooling"), manhattan_plot , 3, 3.5)
