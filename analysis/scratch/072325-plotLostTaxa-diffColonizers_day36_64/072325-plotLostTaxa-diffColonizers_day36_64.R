...
Colonizers",
  ...
  lost_ASVs <- recipient_ASVs %>% 
    filter(biosample1 == recipient_id, replicate == 1, day == "036") %>% 
    ggplot(aes(x = biosample1, y = relAbundance, fill = Family, alpha = factor(lost_strain_36_64)))+
    geom_bar(stat = "identity", color = "black") +
    scale_fill_manual(values = my_colors) +
...
# Function call
subject_composition_plot_day29 <- subject_composition("XDB-029+XBA-036", "XDB-029", "XBA-036", 1) + DEFAULTS.THEME_PRINT
