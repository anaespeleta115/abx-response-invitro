...
get_colonization <- function(mix_id, donor_id, recipient_id, replicate) {
  ...
  donor_asvs_potential <- donor_asvs_subset %>% 
    mutate(potential_colonizer = ifelse(!(OTU %in% recipient_asvs_subset$OTU), 1, 0))
...
potential_col <- donor_asvs_potential %>% 
  filter(potential_colonizer == 1)
  

test <- mix_asvs_colonization %>% 
  filter(actual_colonizer == 1) %>% 
  filter(OTU %in% potential_col$OTU) %>% 
  distinct()
