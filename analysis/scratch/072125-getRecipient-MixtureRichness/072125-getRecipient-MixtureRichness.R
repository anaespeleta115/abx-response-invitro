...
recipient_richness <- recipient_richness %>%
  ...
  count(biosample1, name = "species_richness")  %>% # count the distinct OTUs
...
mixture_richness <- mixture_ASVs %>%
  ...
  select(mixture, OTU) %>%
  count(mixture, name = "species_richness") %>% #count the distinct OTUs
